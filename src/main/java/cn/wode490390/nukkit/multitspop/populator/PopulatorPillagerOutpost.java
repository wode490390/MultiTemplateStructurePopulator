...
public class PopulatorPillagerOutpost extends Populator {
    ...
    protected static final ReadableStructureTemplate WATCHTOWER = new ReadOnlyLegacyStructureTemplate().load(MultiTemplateStructurePopulator.loadNBT("structures/pillageroutpost/watchtower.nbt"));
    protected static final ReadableStructureTemplate WATCHTOWER_OVERGROWN = new ReadOnlyLegacyStructureTemplate().load(MultiTemplateStructurePopulator.loadNBT("structures/pillageroutpost/watchtower_overgrown.nbt"));

    protected static final ReadableStructureTemplate[] FEATURES = new ReadableStructureTemplate[]{
            new ReadOnlyLegacyStructureTemplate().load(MultiTemplateStructurePopulator.loadNBT("structures/pillageroutpost/feature_cage1.nbt")),
            new ReadOnlyLegacyStructureTemplate().load(MultiTemplateStructurePopulator.loadNBT("structures/pillageroutpost/feature_cage2.nbt")),
            new ReadOnlyLegacyStructureTemplate().load(MultiTemplateStructurePopulator.loadNBT("structures/pillageroutpost/feature_logs.nbt")),
            new ReadOnlyLegacyStructureTemplate().load(MultiTemplateStructurePopulator.loadNBT("structures/pillageroutpost/feature_tent1.nbt")),
            new ReadOnlyLegacyStructureTemplate().load(MultiTemplateStructurePopulator.loadNBT("structures/pillageroutpost/feature_tent2.nbt")),
            new ReadOnlyLegacyStructureTemplate().load(MultiTemplateStructurePopulator.loadNBT("structures/pillageroutpost/feature_targets.nbt"))
    };
            ...
            if (nbt.getString("id").equals(BlockActorId.STRUCTURE_BLOCK)) {
                switch (nbt.getString("metadata")) {
                    ...
                    case "cage":
                        Server.getInstance().getScheduler().scheduleDelayedTask(new ActorSpawnTask(chunk.getProvider().getLevel(),
                                Entity.getDefaultNBT(new Vector3(nbt.getInt("x") + 0.5, nbt.getInt("y"), nbt.getInt("z") + 0.5))
                                        .putString("id", String.valueOf(ActorId.IRON_GOLEM))), 2);
                        break;
                }
            }
        };
